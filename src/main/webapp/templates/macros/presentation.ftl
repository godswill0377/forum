...
<#macro forumsComboTable>
    ...
    <div class="form-group goSection">

        <form action="" name="f" id="f" accept-charset="${encoding}">
            <label for="sel1">Go to:</label>
            <select class="goSection-control" id="sel1" onchange="if(this.options[this.selectedIndex].value != -1){ document.location = '${contextPath}/forums/show/'+ this.options[this.selectedIndex].value +'${extension}'; }" name="select">
            ...
            <button type="button"
                    onclick="if(document.f.select.options[document.f.select.selectedIndex].value != -1){ document.location = '${contextPath}/forums/show/'+ document.f.select.options[document.f.select.selectedIndex].value +'${extension}'; }" class="btn">${I18n.getMessage("ForumIndex.goTo")}</button>
